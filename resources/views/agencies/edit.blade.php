  ...
  <main class="flex-1 bg-white">
    <div class="px-12 py-10">
      <div class="flex items-center gap-4">
        <a href="{{ route('agencies.index') }}" class="text-gray-400 hover:text-gray-600">
        ...
        <h1 class="text-2xl font-semibold text-gray-900">Edit Agency</h1>
      </div>
      <div class="mt-6 h-px w-full bg-gray-100"></div>

      <form action="{{ route('agencies.update', $agency) }}" method="POST" enctype="multipart/form-data" class="mt-8 max-w-2xl space-y-8">
        @csrf
        @method('PUT')
        ...
        <div class="space-y-6">
          ...
          <div>
            ...
            <div class="mt-2">
              @if($agency->company_logo)
                <div class="mb-3 flex items-center gap-4">
                  <img src="{{ Storage::url($agency->company_logo) }}" alt="{{ $agency->company_name }}" class="max-h-16 rounded-lg object-contain" />
            ...
            <p class="mt-1 text-xs text-gray-500">JPEG or PNG, max 1 MB.{{ $agency->company_logo ? ' Upload a new file to replace the current logo.' : '' }}</p>
            @error('company_logo')
          ...
          <div>
            ...
            <div class="mt-2">
              <input type="text" name="company_name" id="company_name" value="{{ old('company_name', $agency->company_name) }}" required
          ...
          <div class="grid grid-cols-2 gap-6">
            <div>
              ...
              <div class="mt-2">
                <input type="text" name="brn" id="brn" value="{{ old('brn', $agency->brn) }}" required
          ...
          <div class="grid grid-cols-2 gap-6">
            ...
            <div class="flex items-end pb-1">
              <label class="inline-flex items-center gap-2 text-sm text-gray-700">
                ...
                <input type="checkbox" name="vat_exempt" value="1" {{ old('vat_exempt', $agency->vat_exempt) ? 'checked' : '' }}
        ...
        <div class="space-y-6">
              ...
              <div class="mt-2">
                <select name="commission_type" id="commission_type"
                  ...
                  @foreach($commissionTypes as $type)
                    <option value="{{ $type->value }}" {{ old('commission_type', $agency->commission_type?->value) === $type->value ? 'selected' : '' }}>{{ $type->value }}</option>
                  @endforeach
                </select>
              </div>
          ...
          <div class="grid grid-cols-2 gap-6">
            <div>
              ...
              <div class="mt-2">
                <input name="discount" id="discount" value="{{ old('discount', $agency->discount) }}" min="0"
              ...
              <div class="mt-2">
                <select name="discount_type" id="discount_type"
                  ...
                  @foreach($discountTypes as $type)
                    <option value="{{ $type->value }}" {{ old('discount_type', $agency->discount_type?->value) === $type->value ? 'selected' : '' }}>{{ $type->value }}</option>
                  @endforeach
                </select>
              </div>
              @error('discount_type')
        ...
        <div class="space-y-6">
          ...
          <div>
            ...
            <div class="mt-2">
              <input type="text" name="contact_person_name" id="contact_person_name" value="{{ old('contact_person_name', $agency->contact_person_name) }}"
          ...
          <div class="grid grid-cols-2 gap-6">
            <div>
              ...
              <div class="mt-2">
                <input type="email" name="contact_person_email" id="contact_person_email" value="{{ old('contact_person_email', $agency->contact_person_email) }}"
            ...
            <div>
              ...
              <div class="mt-2">
                <input type="text" name="contact_person_phone" id="contact_person_phone" value="{{ old('contact_person_phone', $agency->contact_person_phone) }}"
